...
(function () {
  ...
  function injectStyles() {
    if (document.getElementById('wnmuMainLibraryUiV15101Styles')) return;
    const style = document.createElement('style');
    style.id = 'wnmuMainLibraryUiV15101Styles';
    style.textContent = `
      #appShell .topbar {
      ...
      #controlsPanel .program-type-toggle button.active {
        background: linear-gradient(180deg, var(--pbs-blue-dark), var(--pbs-blue)) !important;
        color: #fff !important;
      }
  ...
  function syncVisibleVersionFlag() {
    ...
    if (pill) pill.textContent = 'v1.5.101';
  }
  ...
  function stackClearAllButton() {
    ...
    button.innerHTML = '<span>Clear</span><span>all</span><span>filters</span>';
  }
  ...
  function applyFilterSizingLayout() {
    ...
    setImportant(cluster, 'grid-template-columns', 'minmax(160px,1.45fr) minmax(120px,2.35fr) minmax(72px,.7fr) minmax(100px,1fr) minmax(46px,.42fr) minmax(90px,.8fr) minmax(100px,1.25fr) minmax(100px,1.25fr)');
    ...
    if (distributor) {
      setImportant(distributor, 'max-width', '96px');
    }
    ...
    if (clearHolder) {
      ...
      setImportant(clearHolder, 'padding-top', '14px');
      setImportant(clearHolder, 'max-width', '54px');
    }
  }
  ...
  function patchRenderTable() {
    if (window.__wnmuMainUiV15101RenderPatch) return;
    window.__wnmuMainUiV15101RenderPatch = true;
  ...
  function patchRenderFilters() {
    if (window.__wnmuMainUiV15101FilterPatch) return;
    window.__wnmuMainUiV15101FilterPatch = true;
